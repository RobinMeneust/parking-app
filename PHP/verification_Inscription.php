...
foreach($users as $user) {
	if($user->email == $_POST["email"]) {
		header('location:../Inscription_Connexion.php?message=Cet email est déjà utilisé.&form=Register');
		exit;
	}
}
...
if ( $_SESSION["VAR_profil"]["prenom"] == "error" || $_SESSION["VAR_profil"]["nom"] == "error" ||   $_SESSION["VAR_profil"]["password"] == "error" || $_SESSION["VAR_profil"]["email"]  == "error" || $_SESSION["VAR_profil"]["adresse"]  == "error"|| $_SESSION["VAR_profil"]["code_postal"] =="error" || $_SESSION["VAR_profil"]["valid_conditions"] =="error" ) {
    ...
    array_push($users, ["prenom"=> $_POST["prenom"], "nom"=> $_POST["nom"], "password"=> password_hash($_POST["password"], PASSWORD_BCRYPT), "email"=> $_POST["email"],"adresse"=> $_POST["adresse"], "code_postal"=> $_POST["code_postal"], "valid_conditions"=> $_POST["valid_conditions"]]);
    $res = [];
    $res["users"] = $users;
